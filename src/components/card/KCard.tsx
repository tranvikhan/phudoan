...
export default function KCard({ type, image, heading, paragraph }: IKCard) {
  if (type === "overlay")
    return (
      <div className="w-full h-40 relative" data-aos="fade-up">
        <div className="object-cover h-full w-full filter brightness-75">
          <Image
            src={image ? image : ""}
            alt={heading}
            layout="fill"
    ...
    return (
      <div className="rounded shadow w-full p-4" data-aos="flip-left">
        <img
          src={image ? image : ""}
          alt={heading}
          className="object-cover xl:h-40 h-16 w-full"
        />
      </div>
    );
  }
  ...
  return (
    <div className="w-full xl:h-64 lg:52 h-40 relative" data-aos="fade-up">
      <img
        src={image ? image : ""}
        alt={heading}
        className="object-cover xl:h-64 lg:52 h-40 w-full"
      />
    </div>
  );
}
